...
def get_music(song_id,song_name):
	headers = {'user-agent':'Mozilla/5.0'}
	url = 'http://music.163.com/song/media/outer/url?id=' + str(song_id) + '.mp3'
	try:
		r = requests.get(url,headers = headers)
		#print(r.status_code)
		with open('C:\\Users\\dell\\Desktop\\网易云抓包\\hot_music\\' + song_name + '.mp3','wb') as m:
			m.write(r.content)
			m.close()
		print(song_name + ">>>写入完成",'\n')
	except:
		print(song_name +'>>>写入失败')
		pass
	return None
...
def Word_Frequency():
    ...
    dict = {}
    filename_list = ['hot_music_lyric.txt','hot_music_comment.txt']
    useless_char = ['作曲','作词','the','it','to','on','be','dict','keys','content','nickname','values','and','na','in','that','re','ll','don','01.00','Oh','And','oh','00.00','so']
	  #排除词汇列表
    for name in filename_list:
        ...
        for word in words:
            ...
            if word.isdigit():
                continue
            if word in useless_char:
				#words.remove(word)  #这里如果使用remove方法，会大幅降低程序效率
                continue
            if len(word) == 1:
				#words.remove(word)
                continue
        ...
        print(name[:-4] + ':\n')
        for i in range(10):  #输出频率最高的10个词语及出现次数
            word,num = kvs[i]
